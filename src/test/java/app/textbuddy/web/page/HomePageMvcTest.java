...
class HomePageMvcTest {
    ...
    void getRootRendersEditorIslandShell() throws Exception {
        ...
        mockMvc.perform(get("/"))
                ...
                .andExpect(content().string(containsString("accept=\".pdf,.docx,.pptx,.xlsx,.html,.md,.adoc,.txt\"")))
                .andExpect(content().string(containsString("Unterstützte Dokumente werden standardmässig eingebettet über Kreuzberg verarbeitet.")))
                .andExpect(content().string(containsString("Bei aktiver OIDC-Konfiguration läuft der Endpoint nur für angemeldete Sessions.")))
                ...
                .andExpect(content().string(containsString("Bubble-Menü")))
