...
| `POST /api/convert/doc` | Multipart/JSON | Dokument in bereinigtes HTML umwandeln |
...
- Adapter führen keine automatischen Wiederholungen aus. Das vermeidet unbemerkte Mehrfachkosten und schwer vorhersagbare Latenz.

## Sicherheitsgrenzen

Im Produktionsmodus schützt OIDC alle APIs. Zustandsändernde Browseranfragen tragen ein CSRF-Token. Ein ungeschützter Start ist nur bei expliziter Loopback-Bindung möglich. Upload-, Text- und Promptgrössen werden vor teurer Verarbeitung geprüft. CSP und `X-Frame-Options: SAMEORIGIN` erlauben den eigenen PDF-Viewer, aber keine fremde Einbettung.
